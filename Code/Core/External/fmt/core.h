...
template <typename TChar> using std_string_view = std::basic_string_view<TChar>;
#elif defined(FMT_USE_EXPERIMENTAL_STRING_VIEW)
template <typename TChar>
using std_string_view = std::experimental::basic_string_view<TChar>;
#else
...
template <typename TChar> class basic_string_view {
 ...
 public:
  ...
  FMT_CONSTEXPR basic_string_view(const TChar* s, size_t count) FMT_NOEXCEPT
      : data_(s),
        size_(count) {}
  ...
  /**
    ...
    the size with ``std::char_traits<TChar>::length``.
    \endrst
   */
  basic_string_view(const TChar* s)
      : data_(s), size_(std::char_traits<TChar>::length(s)) {}

  /** Constructs a string reference from a ``std::basic_string`` object. */
  template <typename Traits, typename Alloc>
  FMT_CONSTEXPR basic_string_view(
      const std::basic_string<TChar, Traits, Alloc>& s) FMT_NOEXCEPT
      : data_(s.data()),
        size_(s.size()) {}

  template <
      typename S,
      FMT_ENABLE_IF(std::is_same<S, internal::std_string_view<TChar>>::value)>
  FMT_CONSTEXPR basic_string_view(S s) FMT_NOEXCEPT : data_(s.data()),
                                                      size_(s.size()) {}

  /** Returns a pointer to the string data. */
  FMT_CONSTEXPR const TChar* data() const { return data_; }
  ...
  FMT_CONSTEXPR const TChar& operator[](size_t pos) const { return data_[pos]; }
  ...
  int compare(basic_string_view other) const {
    ...
    int result = std::char_traits<TChar>::compare(data_, other.data_, str_size);
    if (result == 0)
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename TChar, FMT_ENABLE_IF(is_char<TChar>::value)>
inline basic_string_view<TChar> to_string_view(const TChar* s) {
  return s;
}

template <typename TChar, typename Traits, typename Alloc>
inline basic_string_view<TChar> to_string_view(
    const std::basic_string<TChar, Traits, Alloc>& s) {
  return s;
}

template <typename TChar>
inline basic_string_view<TChar> to_string_view(basic_string_view<TChar> s) {
  return s;
}

template <typename TChar,
          FMT_ENABLE_IF(!std::is_empty<internal::std_string_view<TChar>>::value)>
inline basic_string_view<TChar> to_string_view(
    internal::std_string_view<TChar> s) {
  return s;
}
...
/**
  ...
  \endrst
 */
template <typename TChar, typename ErrorHandler = internal::error_handler>
class basic_format_parse_context : private ErrorHandler {
 private:
  basic_string_view<TChar> format_str_;
  int next_arg_id_;

 public:
  using char_type = TChar;
  using iterator = typename basic_string_view<TChar>::iterator;

  explicit FMT_CONSTEXPR basic_format_parse_context(
      basic_string_view<TChar> format_str, ErrorHandler eh = ErrorHandler())
  ...
  FMT_CONSTEXPR void check_arg_id(basic_string_view<TChar>) {}
...
using basic_parse_context FMT_DEPRECATED_ALIAS =
    basic_format_parse_context<TChar, ErrorHandler>;
...
template <typename T, typename TChar = char, typename Enable = void>
struct formatter {
  // A deleted default constructor indicates a disabled formatter.
  formatter() = delete;
};

template <typename T, typename TChar, typename Enable = void>
...
template <typename TChar> struct named_arg_base;
template <typename T, typename TChar> struct named_arg;
...
#define FMT_TYPE_CONSTANT(Type, constant) \
  template <typename TChar>                \
  struct type_constant<Type, TChar> : std::integral_constant<type, constant> {}

FMT_TYPE_CONSTANT(const named_arg_base<TChar>&, named_arg_type);
...
FMT_TYPE_CONSTANT(TChar, char_type);
FMT_TYPE_CONSTANT(float, float_type);
FMT_TYPE_CONSTANT(double, double_type);
FMT_TYPE_CONSTANT(long double, long_double_type);
FMT_TYPE_CONSTANT(const TChar*, cstring_type);
FMT_TYPE_CONSTANT(basic_string_view<TChar>, string_type);
...
template <typename TChar> struct string_value {
  const TChar* data;
  std::size_t size;
};
...
// Formatting context.
template <typename OutputIt, typename TChar> class basic_format_context {
 public:
...
using buffer_context =
    basic_format_context<std::back_insert_iterator<internal::buffer<TChar>>,
                         TChar>;
...
template <typename TChar>
struct is_contiguous<std::basic_string<TChar>> : std::true_type {};

template <typename TChar>
struct is_contiguous<internal::buffer<TChar>> : std::true_type {};
...
template <typename TChar> struct named_arg_base {
  basic_string_view<TChar> name;

  // Serialized value<context>.
  mutable char data[sizeof(basic_format_arg<buffer_context<TChar>>)];

  named_arg_base(basic_string_view<TChar> nm) : name(nm) {}
...
template <typename T, typename TChar> struct named_arg : named_arg_base<TChar> {
  const T& value;

  named_arg(basic_string_view<TChar> name, const T& val)
      : named_arg_base<TChar>(name), value(val) {}
};
...
template <typename... Args, typename S, typename TChar = char_t<S>>
inline format_arg_store<buffer_context<TChar>, remove_reference_t<Args>...>
...
template <typename TChar>
std::basic_string<TChar> vformat(basic_string_view<TChar> format_str,
                                basic_format_args<buffer_context<TChar>> args);

template <typename TChar>
typename buffer_context<TChar>::iterator vformat_to(
    buffer<TChar>& buf, basic_string_view<TChar> format_str,
    basic_format_args<buffer_context<TChar>> args);
}  // namespace internal
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename S, typename T, typename TChar = char_t<S>>
inline internal::named_arg<T, TChar> arg(const S& name, const T& arg) {
  static_assert(internal::is_string<S>::value, "");
  return {name, arg};
}

// Disable nested named arguments, e.g. ``arg("a", arg("b", 42))``.
template <typename S, typename T, typename TChar>
void arg(S, internal::named_arg<T, TChar>) = delete;

/** Formats a string and writes the output to ``out``. */
// GCC 8 and earlier cannot handle std::back_insert_iterator<Container> with
// vformat_to<ArgFormatter>(...) overload, so SFINAE on iterator type instead.
template <typename OutputIt, typename S, typename TChar = char_t<S>,
          FMT_ENABLE_IF(
...
template <typename S, typename TChar = char_t<S>>
inline std::basic_string<TChar> vformat(
    const S& format_str, basic_format_args<buffer_context<TChar>> args) {
...
template <typename S, typename... Args, typename TChar = char_t<S>>
inline std::basic_string<TChar> format(const S& format_str, Args&&... args) {
